...
public class gameManager : MonoBehaviour {
	...
	void initializeCards() {
		...
		List<int> cardValues = new List<int> (cardFace.Length/2);
		for(int i=0;i<cardFace.Length; i++) { cardValues.Add (i+1); cardValues.Add(i+1); }
        foreach (var card in cards)
        {
			int choice=Random.Range(0, cardValues.Count);
			card.GetComponent<cardScript>().cardValue = cardValues[choice];
            card.GetComponent<cardScript>().initialized = true;
            cardValues.RemoveAt(choice);
        }

        /*
		for (int id = 0; id < 2; id++) {
			for (int i = 1; i <=cardFace.Length/2; i++) {
